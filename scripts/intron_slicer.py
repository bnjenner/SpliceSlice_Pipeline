...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to file with list of transcripts of interest.",
    )
    parser.add_argument(
        "-a",
        "--annotation",
    ...
    parser.add_argument(
        ...
        default=200,
        required=False,
        help="Length of window for Branch Point sequences.",
    )
    parser.add_argument(
        "-u",
        "--utr-window",
        ...
        default=150,
        required=False,
        help="Length of window for 5' UTR.",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        required=False,
        help="Path to transcripts of interest output file.",
    )
    parser.add_argument(
        "--training",
        action="store_true",
        help="Extract BPs, PPTs, and Background sequences for training.",
    )
            ...
            if not gtf_lines[i].startswith("#"):
                ...
                if cols[2] == "transcript":
                    ...
                    if next((a for a in anno if "exon_number 1;" in a), False):
                        ...
                        if strand == "+":
                            gtf_dict[transcript_id]["introns"].extend(
                                [int(cols[3]), int(cols[4])]
                            )
                        else:
                            gtf_dict[transcript_id]["introns"].extend(
                                [int(cols[4]), int(cols[3])]
                            )
